...
from as2fm.scxml_converter.scxml_entries import ScxmlParam, ScxmlSend, ScxmlBase
from as2fm.scxml_converter.ascxml_extensions import AscxmlDeclaration
from as2fm.scxml_converter.ascxml_extensions.bt_entries import BtOutputPortDeclaration
...
class BtSetValueOutputPort(ScxmlSend):
    ...
    def as_plain_scxml(self, struct_declarations, ascxml_declarations: List[AscxmlDeclaration], **kwargs)->List[ScxmlBase]:
        # Find the related BT Output port declaration
        for ascxml_decl in ascxml_declarations:
            if isinstance(ascxml_decl, BtOutputPortDeclaration):
                if ascxml_decl.get_key_name() == self._key:
                    port_value = ascxml_decl.get_key_value()
                    break
        assert port_value is not None  # MyPy check
        check_assertion(is_blackboard_reference(port_value), self.get_xml_origin(), f"BT Port {self._key} is expected to be a blackboard variable.")
        return [
            ScxmlSend(
