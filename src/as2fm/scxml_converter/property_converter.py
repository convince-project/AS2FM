...
class PropertyConverter:
    ...
    def __init__(
        ...
        self._assumes_node: Optional[XmlElement] = root.find("assumes")
        self._guarantees_node: Optional[XmlElement] = root.find("guarantees")
        assert (
    ...
    def _process_properties(
        self, output_properties: XmlElement, input_properties: XmlElement
    ) -> None:
                ...
                if tag == "time_interval":
                    ...
                    if child.attrib.get("time") is None:
                        ...
                        if after is not None:
                            after = self._convert_time(
                                after, property_time_unit, self._model_time_unit
                            )
                        if within is not None:
                            within = self._convert_time(
                                within, property_time_unit, self._model_time_unit
                            )
                        time = (after, within)
                    else:
