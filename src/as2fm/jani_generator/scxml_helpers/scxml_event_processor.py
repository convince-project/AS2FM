...
def implement_scxml_events_as_jani_syncs(
    events_holder: EventsHolder, timers: List[RosTimer], max_array_size: int, jani_model: JaniModel
) -> List[str]:
    """
    ...
    for event_name, event in events_holder.get_events().items():
                ...
                JaniEdge(
                    {
                        "location": "waiting",
                        "destinations": [
                            {"location": "waiting", "probability": {"exp": 1.0}, "assignments": []}
                        ],
                ...
                JaniEdge(
                    {
                        "location": "waiting",
                        "destinations": [
                            {"location": "received", "probability": {"exp": 1.0}, "assignments": []}
                        ],
        ...
        if event.has_receivers():
            ...
            for receiver in event.get_receivers():
                ...
                assert (
                    action_name == event_name_on_receive
        ...
        for sender in event.get_senders():
            ...
            assert (
                action_name == event_name_on_send
            ), f"Action name {action_name} must be {event_name_on_send}."
            senders_syncs = {event_name: action_name, sender.automaton_name: action_name}
        ...
        jani_model.add_variable(
            variable_name=f"{event_name}.valid", variable_type=bool, variable_init_expression=False
        )
    ...
    for timer in timers:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"{ROS_TIMER_RATE_EVENT_PREFIX}{name}."
            ) from e
