...
def __generate_nested_array_access_expr(
    array_name: str, access_indexes: Union[List[JaniExpression], List[int]]
) -> JaniExpression:
    """
...
def __assign_array_dimensions_from_variable(
    ...
    target_variable_indexes: List[int],
    target_variable_array_info: ArrayInfo,
    assigned_variable_name: str,
    assigned_variable_indexes: List[int],
    assignment_index: int,
) -> List[JaniAssignment]:
    """
    Update the autogenerated JANI entries tracking the runtime array size.

    This function implement the case of an array variable assigned to another array variable.
    """
...
def __assign_array_dimensions_from_expression(
    ...
    This function implement the case of a constant array expression assigned to an array variable.
    """
    target_used_levels = len(target_variable_indexes)
    target_total_levels = len(target_variable_array_info.array_max_sizes)
    remaining_levels = target_total_levels - target_used_levels
    assert remaining_levels >= 0, get_error_msg(
        elem_xml, f"Trying to access a non-existing dimension of {target_variable_name}."
    )
    assert remaining_levels > 0, get_error_msg(
        elem_xml, f"Trying to assign an array to a base type on {target_variable_name}."
    )
    new_assignments: List[JaniAssignment] = []
    assign_expr_as_list = get_array_expr_as_list(assign_expression_str, elem_xml)
...
def generate_jani_assignments(
    ...
    if target_expr_type is JaniExpressionType.OPERATOR:
        ...
        if remaining_dimensions > 0:
            ...
            if assignment_value_type == JaniExpressionType.IDENTIFIER:
                ...
                if assignment_value.op == "aa":
                    ...
                    raise RuntimeError("Unexpected operator found: '{assignment_value.op}'")
            else:
                raise RuntimeError("Cannot assign Literals to an array entry.")
    else:
        ...
        assignment_target_name = assignment_target_var.name()

        target_array_info = assignment_target_var.get_array_info()
        assignment_value = parse_ecmascript_to_jani_expression(
            assign_expr, elem_xml, target_array_info
        ).replace_event(event_substitution)
        assignments.append(
            JaniAssignment(
                {"ref": assignment_target_name, "value": assignment_value, "index": assign_index}
            )
        )
        # In case this is an array assignment, the length must be adapted too
        if is_variable_array(assignment_target_var):
            assert target_array_info is not None, get_error_msg(
                elem_xml, f"Expected target_array_info to be available for {assignment_target_name}"
            )
            assignment_value_type = assignment_value.get_expression_type()
                ...
                if operator_type in ("ac", "av"):
                    ...
                    raise RuntimeError(
                        get_error_msg(
                            ...
                            f"{assignment_target_name}.",
                        )
                    )
            else:
                assignment_value_name = assignment_value.as_identifier()  # type: ignore
                assert assignment_value_name is not None, get_error_msg(
