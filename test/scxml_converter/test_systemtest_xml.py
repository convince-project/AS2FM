...
def ros_to_plain_scxml_test(test_folder: str,
    ...
    for fname in scxml_files:
        ...
        try:
            ...
            assert len(plain_scxmls) == len(gt_files), \
            ...
            for generated_scxml in plain_scxmls:
                ...
                assert scxml_object_name in gt_files, \
                    f"Generated SCXML {scxml_object_name} not in gt SCXMLs {gt_files}."
                gt_file_path = os.path.join(test_data_path, 'gt_plain_scxml',
                                            f'{scxml_object_name}.scxml')
                with open(gt_file_path, 'r', encoding='utf-8') as f_o:
                    gt_output = f_o.read()
                assert remove_empty_lines(canonicalize_xml(generated_scxml.as_xml_string())) == \
