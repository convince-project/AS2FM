...
def test_traces():
    ...
    assert ver == 73, f"The id of the first verified trace must be 73 but is {ver}."
    assert fal == 0, f"The id of the first falsified trace must be 0 but is {fal}."
    ...
    for i, fname_expected in [
        ...
        assert not os.path.exists(output_file), f"The file {output_file} already exists."
        traces.write_trace_to_img(i, output_file)
        assert os.path.exists(output_file), f"The file {output_file} was not created."
        path_expected = os.path.join(test_data_folder, "expected_output", fname_expected)
        with open(output_file, "rb") as f1, open(path_expected, "rb") as f2:
            assert f1.read() == f2.read(), f"The content for {fname_expected} is not as expected."
