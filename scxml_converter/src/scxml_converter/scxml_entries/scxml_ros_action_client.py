...
class RosActionHandleGoalResponse(ScxmlTransition):
    ...
    def __init__(self, action_client: Union[str, RosActionClient],
                 accept_target: str, reject_target: str) -> None:
        """
    ...
    def check_validity(self) -> bool:
        ...
        valid_accept = is_non_empty_string(RosActionHandleGoalResponse, "accept",
                                           self._accept_target)
        valid_reject = is_non_empty_string(RosActionHandleGoalResponse, "reject",
                                           self._reject_target)
    ...
    def as_plain_scxml(self,
                       ros_declarations: ScxmlRosDeclarationsContainer) -> List[ScxmlTransition]:
        assert self.check_valid_ros_instantiations(ros_declarations), \
    ...
    def as_xml(self) -> ET.Element:
        ...
        return ET.Element(RosActionHandleGoalResponse.get_tag_name(),
                          {"name": self._client_name,
                           "accept": self._accept_target, "reject": self._reject_target})
...
class RosActionHandleFeedback(ScxmlTransition):
    ...
    def from_xml_tree(xml_tree: ET.Element) -> "RosActionHandleFeedback":
        """Create a RosActionHandleFeedback object from an XML tree."""
        assert_xml_tag_ok(RosActionHandleFeedback, xml_tree)
        client_name = get_xml_argument(RosActionHandleFeedback, xml_tree, "name")
        target_name = get_xml_argument(RosActionHandleFeedback, xml_tree, "target")
        exec_body = execution_body_from_xml(xml_tree)
        return RosActionHandleFeedback(client_name, target_name, exec_body)

    def __init__(self, action_client: Union[str, RosActionClient], target: str,
                 body: Optional[ScxmlExecutionBody] = None) -> None:
        """
        Initialize a new RosActionHandleFeedback object.
        ...
        if isinstance(action_client, RosActionClient):
            ...
            assert is_non_empty_string(RosActionHandleFeedback, "name", action_client)
            self._client_name = action_client
        self._target = target
        self._body = body
        assert self.check_validity(), "Error: SCXML RosActionHandleFeedback: invalid parameters."

    def check_validity(self) -> bool:
        valid_name = is_non_empty_string(RosActionHandleFeedback, "name", self._client_name)
        valid_target = is_non_empty_string(RosActionHandleFeedback, "target", self._target)
        valid_body = self._body is None or valid_execution_body(self._body)
        if not valid_body:
            print("Error: SCXML RosActionHandleFeedback: body is not valid.")
        return valid_name and valid_target and valid_body

    def check_valid_ros_instantiations(self,
                                       ros_declarations: ScxmlRosDeclarationsContainer) -> bool:
        assert isinstance(ros_declarations, ScxmlRosDeclarationsContainer), \
            "Error: SCXML RosActionHandleFeedback: invalid ROS declarations container."
        if not ros_declarations.is_action_client_defined(self._client_name):
            print("Error: SCXML RosActionHandleFeedback: "
                  f"action client {self._client_name} not declared.")
            return False
        if not super().check_valid_ros_instantiations(ros_declarations):
            print("Error: SCXML RosActionHandleFeedback: invalid ROS instantiations in exec body.")
            return False
        return True
    ...
    def as_plain_scxml(self, ros_declarations: ScxmlRosDeclarationsContainer) -> ScxmlTransition:
        ...
        automaton_name = ros_declarations.get_automaton_name()
        interface_name, _ = ros_declarations.get_action_client_info(self._client_name)
        event_name = generate_action_feedback_handle_event(interface_name, automaton_name)
        target = self._target
    ...
    def as_xml(self) -> ET.Element:
        assert self.check_validity(), "Error: SCXML RosActionHandleFeedback: invalid parameters."
        xml_handle_feedback = ET.Element(RosActionHandleFeedback.get_tag_name(),
